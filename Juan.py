...
class juan(Sprite):
    def __init__(self, muros_grupo, x_inicial, y_inicial):
        ...
        self.delay_inicio = 7000
        self.tiempo_creacion = pygame.time.get_ticks()
        self.puede_salir = False

        # Cargar imágenes normales según dirección
        direccion_rutas = {
        ...
        # Superpoder (azules) – sin dirección
        self.imagenes_super = []
        ...
        # Parpadeo final (blanco) – sin dirección
        ...
        if self.super_modo:
            if self.parpadeo:
                ...
                imagen_actual = self.imagenes_super[self.anim_frame % len(self.imagenes_super)] \
                    if self.parpadeo_frame == 0 else \
                    self.imagenes_parpadeo[self.anim_frame % len(self.imagenes_parpadeo)]
            else:
                imagen_actual = self.imagenes_super[self.anim_frame % len(self.imagenes_super)]
        else:
